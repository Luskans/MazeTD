...
export class FloatingTextService {
  ...
  public object(x: number, y: number, action: string,  cost: string) {
    if (!this.scene) return;
    ...
    const price = (action === "Sold") ? `+ ${cost}` : `- ${cost}`;
    const text2 = this.scene.add.text(-10, 16, price, {
      fontFamily: 'Roboto',
      fontSize: '28px',
      fontStyle: 'bold',
      color: (action === "Sold") ? COLORS.SUCCESS : COLORS.ERROR,
      stroke: COLORS.OUTLINE,
      strokeThickness: 4
    }).setOrigin(0.5).setScale(0.5);

    const icon = this.scene.add.image(15, 16, 'income').setDisplaySize(18, 18).setOrigin(0.5, 0.5);
  ...
  public upgrade(action: string, cost: string) {
    if (!this.scene) return;
    ...
    const container = this.scene.add.container(centerX, topY, [text, text2, icon]).setDepth(10000);
